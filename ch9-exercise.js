...
console.log(logFilename, logFilename.unsafePerformIO())
...
const emailBlast = (emails) =>
  new IO(() => emails.forEach((email) => console.log(`notice ${email}`)))
...
const joinMailingList = compose(
  map(compose(join, map(emailBlast), addToMailingList)),
  validateEmail
)
